...
public:
    ...
    const year_month_day &getDob() const
    {
        return dob;
    }
...
public:
    ...
    Student(year_month_day dob, string_view familyName = "DefaultFam", string_view firstName = "DefaultFirName", bool familyNameFirst = false, const vector<string> &attended_classes = {}, Schooling school_type = Schooling::preschool)
        : Person{dob, familyName, firstName, familyNameFirst}, school_type{school_type}, attended_classes{attended_classes} {};
    ...
    const Schooling getSchooling() const { return school_type; }
    void setSchooling(Schooling school_type)
    {
...
int main()
{
    Student s{{{1879y, March, 14d}, "Einstein", "Albert"}, {"math", "physics", "philosophy"}, Student::Schooling::other};
    Employee e{{{1879y, March, 14d}, "Einstein", "Albert"}, 1001, 15000};

    cout << s.getDob() << s.getName() << (int)s.getSchooling() << '\n';
    s.setFamilyName("NewFamName");
    s.setFirstName("NewFirstName");
    s.setFamilyNameFirst(true);
    s.setSchooling(Student::Schooling::college);
    cout << s.getDob() << s.getName() << (int)s.getSchooling() << '\n';

    Student s2{{1879y, March, 14d}};
    cout << s2.getDob() << s2.getName() << (int)s2.getSchooling() << '\n';

    Employee e2{{{1879y, March, 14d}}, 1};
    cout << e2.getDob();
}